...
//setting
#define PRIORITY_MAX_LENGTH 500005
#define NODE_MAX 1005
#define EDGE_MAX 10005
...
void priority_queue_push(struct priority_queue* priority_Q, struct dijkstraNode newItem) {
    priority_Q->size++;
    priority_Q->arr[priority_Q->size] = newItem;
    int nowLocation = priority_Q->size;
    while (nowLocation > 1) {
        int parentLocation = nowLocation / 2;
        if (priority_queue_compare(priority_Q->arr[parentLocation], priority_Q->arr[nowLocation])) {
            priority_queue_swap(&priority_Q->arr[parentLocation], &priority_Q->arr[nowLocation]);
            nowLocation /= 2;
        }
        else {
            break;
